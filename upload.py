...
def upload(path, url):
    ...
    tnotification(path, "设置图片信息", "您有一张新截图,图片已上传,单击此处来设置该图片在时间线中的信息",
                  'python3 {} -i "{}" -p "{}" '.format('/data/data/com.termux/files/home/.shortcuts/upload.py',
                                                       result['filename'], result['bag']), button1='删除', button1_action='python3 {} -d "{}" -p "{}" '
                  .format('/data/data/com.termux/files/home/.shortcuts/upload.py', result['filename'], result['bag']))
...
parser.add_argument('-s', '--scan', action='store_true',
                    help='先扫描路径下已有文件，可与watch选项配合使用')
parser.add_argument('-w', '--watch', action='store_true',
                    help='忽略已有文件，持续扫描，可和scan选项配合使用')
